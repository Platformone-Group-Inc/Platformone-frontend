...
export const getControlByControlFamiliesQueryFn = async (
  ...
  if (all && frameworkId) {
    ...
    throw new Error("Either controlFamilyId or frameworkId with all=true must be provided.");
  }

  params.append("page", page.toString());
  params.append("limit", limit.toString());

  const response = await API.get(`/controls/get-controls-by-family?${params.toString()}`);
  return response?.data;
};
